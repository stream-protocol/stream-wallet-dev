    ...
    try {
        ...
        if (foundedVersion) {
            ...
            if (semantic) {
                ...
                await $ `cd packages/extension/build/chrome && zip -r stream-wallet-extension-${foundedVersion}.zip .`;
                await $ `cd packages/extension/build/firefox && zip -r stream-wallet-extension-${foundedVersion}.firefox.zip .`;
                if (isPrelease) {
                    await $ `gh release create ${foundedVersion} packages/extension/build/chrome/stream-wallet-extension-${foundedVersion}.zip packages/extension/build/firefox/stream-wallet-extension-${foundedVersion}.firefox.zip -t ${foundedVersion} --prerelease`;
                } else {
                    await $ `gh release create ${foundedVersion} packages/extension/build/chrome/stream-wallet-extension-${foundedVersion}.zip packages/extension/build/firefox/stream-wallet-extension-${foundedVersion}.firefox.zip -t ${foundedVersion}`;
                }
