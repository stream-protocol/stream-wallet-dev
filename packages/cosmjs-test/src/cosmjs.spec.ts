...
describe("Test cosmjs compatibility", () => {
  test("test type conflict with offline signer", async () => {
    const stream-wallet = new MockStream(
      () => {
    ...
    const offlineSigner = stream-wallet.getOfflineSignerOnlyAmino("test-1");
  ...
  test("test type conflict with direct signer", async () => {
    ...
    const offlineSigner = stream-wallet.getOfflineSigner("test-1");
