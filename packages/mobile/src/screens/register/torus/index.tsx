  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        finalUrl.searchParams.append(
          "redirect_uri",
          "https://oauth.stream-wallet.app/google.html"
        );

        const result = await WebBrowser.openAuthSessionAsync(
          finalUrl.href,
          "app.stream-wallet.oauth://"
        );
        if (result.type !== "success") {
          throw new Error("Failed to get the oauth");
        }

        if (!result.url.startsWith("app.stream-wallet.oauth://google#")) {
        ...
        if (userResponse.ok) {
          ...
          const {
            ...
            verifier: "stream-protocol-google",
    ...
    (async () => {
      try {
        ...
        const {
          ...
          verifier: "stream-protocol-apple",
          verifierId: sub,
        });
