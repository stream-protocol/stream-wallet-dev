...
import { DenomHelper, escapeHTML, sortObjectByKey } from "@stream-wallet/common";
import { Dec, DecUtils, Int } from "@stream-wallet/unit";
...
import {
  ...
  TxBody,
  TxRaw,
} from "@stream-wallet/proto-types/cosmos/tx/v1beta1/tx";
import { SignMode } from "@stream-wallet/proto-types/cosmos/tx/signing/v1beta1/signing";
import { PubKey } from "@stream-wallet/proto-types/cosmos/crypto/secp256k1/keys";
...
import { VoteOption } from "@stream-wallet/proto-types/cosmos/gov/v1beta1/gov";
import {
  BaseAccount,
  Bech32Address,
  ChainIdHelper,
...
import { ExtensionOptionsWeb3Tx } from "@stream-wallet/proto-types/ethermint/types/v1/web3";
...
export class CosmosAccountImpl {
  ...
  protected async broadcastMsgs(
    ...
    const stream-wallet = (await this.base.getStream())!;
    ...
    if (eip712Signing) {
      if (chainIsInjective) {
        ...
        // There is currently no effective way to handle this in stream-wallet. Just set a very large number.
    ...
    const signResponse: AminoSignResponse = await (async () => {
      if (!eip712Signing) {
        return await stream-wallet.signAmino(
          this.chainId,
          this.base.bech32Address,
          signDoc,
          signOptions
      ...
      return await stream-wallet.experimentalSignEIP712CosmosTx_v0(
        this.chainId,
        this.base.bech32Address,
    ...
    return {
      txHash: await stream-wallet.sendTx(this.chainId, signedTx, mode as BroadcastMode),
