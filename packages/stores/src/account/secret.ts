...
import { MsgExecuteContract } from "@stream-wallet/proto-types/secret/compute/v1beta1/msg";
import { Bech32Address } from "@stream-wallet/cosmos";
import { Dec, DecUtils } from "@stream-wallet/unit";
import { AppCurrency, StreamSignOptions, StdFee } from "@stream-wallet/types";
...
export class SecretAccountImpl {
  ...
  protected async encryptSecretContractMsg(
    ...
    if (!stream-wallet) {
      throw new Error("Can't get the Stream API");
    }

    const enigmaUtils = stream-wallet.getEnigmaUtils(this.chainId);
