...
import { DenomHelper, toGenerator } from "@stream-wallet/common";
...
export interface AccountSetOpts {
  ...
  readonly getStream: () => Promise<Stream | undefined>;
}
...
export class AccountSetBase {
  ...
  getStream(): Promise<Stream | undefined> {
    return this.opts.getStream();
  }
  ...
  protected async enable(stream-wallet: Stream, chainId: string): Promise<void> {
    const chainInfo = this.chainGetter.getChain(chainId);

    if (this.opts.suggestChain) {
      if (this.opts.suggestChainFn) {
        await this.opts.suggestChainFn(stream-wallet, chainInfo);
      } else {
        await this.suggestChain(stream-wallet, chainInfo);
      }
    }
    await stream-wallet.enable(chainId);
  }
  ...
  protected async suggestChain(
    ...
    await stream-wallet.experimentalSuggestChain(chainInfo.raw);
  }
  ...
  public *init() {
    ...
    const stream-wallet = yield* toGenerator(this.getStream());
    if (!stream-wallet) {
      this._walletStatus = WalletStatus.NotExist;
      return;
    }

    this._walletVersion = stream-wallet.version;

    try {
      yield this.enable(stream-wallet, this.chainId);
    } catch (e) {
      console.log(e);
    ...
    try {
      const key = yield* toGenerator(stream-wallet.getKey(this.chainId));
