...
export class Ledger {
  ...
  static async init(
    ...
    try {
      if (app === LedgerApp.Ethereum) {
        ...
        // Ensure that the stream-wallet can connect to ethereum app on ledger.
      ...
      if (versionResponse.deviceLocked) {
        throw new StreamError("ledger", 102, "Device is on screen saver");
      }
  ...
  async getVersion(): Promise<{
    ...
    if (!this.cosmosApp) {
      throw new StreamError("ledger", 100, "Cosmos App not initialized");
    }
  ...
  async getPublicKey(app: LedgerApp, fields: BIP44HDPath): Promise<Uint8Array> {
    if (app === LedgerApp.Ethereum) {
      if (!this.ethereumApp) {
        throw new StreamError("ledger", 100, "Ethereum App not initialized");
      }
