...
import { APP_PORT, Env, StreamError } from "@stream-wallet/router";
      ...
      async (ledger, retryCount: number) => {
        try {
          ...
          if (
            ...
            throw new StreamError("ledger", 110, "Unmatched public key");
          }
  ...
  async initLedger(
    ...
    while (true) {
      ...
      try {
        ...
        if (!transportIniter) {
          throw new StreamError("ledger", 112, `Unknown mode: ${mode}`);
        }
        ...
        try {
          const promises: Promise<unknown>[] = [
            (async () => {
              ...
              if (response?.abort) {
                throw new StreamError("ledger", 120, "Ledger init aborted");
              }
          ...
          promises.push(
            (async () => {
              ...
              if (!timeoutAborted) {
                ...
                throw new StreamError("ledger", 121, "Ledger init timeout");
              }
            })()
          );
